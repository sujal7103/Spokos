...
Shared anti-rate-limit plumbing for every scraper: user-agent rotation, randomised
delays, proxy selection, and a retry decorator.

Proxy sources, in priority order:
- SKOPOS_PROXY       single proxy URL
- SKOPOS_PROXY_FILE  file of proxies, one per line, picked at random
- SKOPOS_FREE_PROXY  free anonymous proxies, cached for 5 minutes

Delays are read from SKOPOS_DELAY_MIN / SKOPOS_DELAY_MAX. All of it is optional -
every scraper works with no proxy configured at all.
"""
...
def get_proxy() -> Optional[str]:
    proxy = os.environ.get('SKOPOS_PROXY')
    if proxy:
        return proxy
    ...
    if os.environ.get('SKOPOS_FREE_PROXY', '').lower() in ('1', 'true', 'yes'):
...
def proxy_status() -> str:
    if os.environ.get('SKOPOS_PROXY'):
        return 'custom'
    if os.environ.get('SKOPOS_PROXY_FILE'):
        return 'file'
    if os.environ.get('SKOPOS_FREE_PROXY', '').lower() in ('1', 'true', 'yes'):
        return 'free'
    return 'none'
