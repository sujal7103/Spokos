...
Fetches profiles through LinkedIn's internal Voyager API using your own session
cookie. This is the only scraper that requires authentication.

Features:
- Reads the li_at cookie from LINKEDIN_COOKIE in .env
- validate_cookie() reports expired or missing cookies before a run
- Caches the client and CSRF token across profiles in one session
- Extracts headline, bio and email where the profile exposes them
"""
...
from skopos.scrapers.stealth import random_user_agent, get_httpx_proxy
from skopos.scrapers.utils import extract_email
