...
Turns a scraped profile into a contactable lead: finds an email address and
scores how much to trust it. No paid API required.

Pipeline:
- Pull email/phone straight out of the bio text
- Deep-scrape the lead's website plus /contact and /about
- Infer the company from the headline, then find its domain via DNS MX lookup
- Detect the site's email pattern (first.last@, first@, ...) and generate candidates
- Verify each candidate against the mail server over SMTP
- Score confidence 0-100 from source and verification result
- Optional Hunter.io lookup for extra coverage when a key is set
"""
...
from skopos.scrapers.stealth import random_user_agent, random_delay
    ...
    def _verify_email_smtp(self, email: str) -> Dict:
        ...
        try:
            with smtplib.SMTP(timeout=10) as smtp:
                ...
                smtp.helo('skopos-verify.local')
                smtp.mail('verify@skopos-verify.local')
